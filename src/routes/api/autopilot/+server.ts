...
} from '$lib/server/agent/autopilot';
...
export async function POST({ request }) {
  ...
  try {
    if (body?.action === 'run')
      return json({ run: await runAutopilotNow(), dashboard: listAutopilotDashboard() });
    if (body?.action === 'save_policy')
      return json({ policy: updateAutopilotPolicy(body.policy || {}) });
