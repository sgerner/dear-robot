...
<section class="fixed bottom-4 right-4 z-40 flex max-h-[86vh] w-[min(720px,calc(100vw-2rem))] flex-col overflow-hidden rounded-lg border border-white/10 bg-zinc-950 shadow-2xl shadow-black/60">
  ...
  <form class="flex min-h-0 flex-1 flex-col" onsubmit={(event) => { event.preventDefault(); sendCompose(); }}>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
          <div class="flex min-w-0 flex-1 items-center gap-2">
            ...
            />
            <DictationButton targetId="compose-ai-prompt" activeTargetId={dictationTargetId} recording={dictationActive} unavailable={dictationUnavailable} level={dictationLevel} onToggle={toggleDictation} />
          </div>
          <button
            type="button"
    ...
    {#if composeEditorMode === 'rich'}
      <div class="relative min-h-0 flex-1">
        ...
        <div class="absolute right-2 top-2">
          <DictationButton targetId="compose-html-body" activeTargetId={dictationTargetId} recording={dictationActive} unavailable={dictationUnavailable} level={dictationLevel} onToggle={toggleDictation} />
        </div>
      </div>
    {:else}
      <div class="relative min-h-0 flex-1">
        ...
        <div class="absolute right-2 top-2">
          <DictationButton targetId="compose-body" activeTargetId={dictationTargetId} recording={dictationActive} unavailable={dictationUnavailable} level={dictationLevel} onToggle={toggleDictation} />
        </div>
      </div>
    {/if}
    <footer class="flex flex-wrap items-center justify-between gap-2 border-t border-white/10 px-4 py-3">
      ...
      <div class="flex gap-2">
        ...
        <button type="button" class="rounded-md border border-white/10 px-3 py-2 text-sm text-zinc-300" onclick={onClose}>Cancel</button>
