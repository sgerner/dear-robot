<script lang="ts">
  ...
  let audioExpanded = $state<boolean | undefined>(undefined);
  ...
  function isProfileExpanded(profileKey: string, profile: { hasApiKey?: boolean } | undefined) {
    return expandedProfiles[profileKey] ?? !profile?.hasApiKey;
  }

  function expandProfile(profileKey: string) {
    expandedProfiles = { ...expandedProfiles, [profileKey]: true };
  }

  function collapseProfile(profileKey: string) {
    expandedProfiles = { ...expandedProfiles, [profileKey]: false };
  }

  function hasSavedAudioKey() {
    return Boolean(data.aiProfiles?.find((item: { profile: string }) => item.profile === 'audio')?.hasApiKey);
  }

  function isAudioExpanded() {
    return audioExpanded ?? !hasSavedAudioKey();
  }
        ...
        <div class="p-5 md:p-6">
          <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
              ...
              {/if}
              {#if profile?.hasApiKey && isProfileExpanded(profileKey, profile)}
                <Button
                  variant="ghost"
                  size="sm"
          ...
          {#if profile?.hasApiKey && !isProfileExpanded(profileKey, profile)}
            <div class="mt-4 flex items-center justify-between gap-3 rounded-xl border border-border/50 bg-muted/30 px-4 py-3">
          ...
          {:else}
            <div class="mt-5 space-y-5" in:fade={{ duration: 140 }}>
              <div class="px-1">
              ...
              {#if profile}
                {#if profileMode[profileKey] === 'catalog'}
                  <div class="space-y-4">
                    ...
                    <div class="grid gap-4">
                      ...
                      <div class="pt-2">
                        <div
                          class="flex items-center justify-between p-3 rounded-lg border border-primary/20 bg-primary/5"
                        >
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 pt-5 border-t border-border/40">
                  <div class="flex items-center gap-2 text-xs text-muted-foreground">
                    <div
                  ...
                  <div class="flex flex-col items-end gap-1.5">
                    ...
                    {#if ((testStates[profileKey] ?? 'idle') === 'error' ||
