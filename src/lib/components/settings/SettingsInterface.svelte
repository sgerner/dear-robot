<script lang="ts">
  let {
    ...
    updateSwipeSetting: (key: 'leftShort' | 'leftLong' | 'rightShort' | 'rightLong', action: any) => void;
  ...
  <section class="rounded-lg border border-white/10 bg-white/[0.03] p-5">
    ...
    <div class="mt-4 space-y-2">
      {#each quickActionCatalog as action (action.id)}
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-md border border-white/10 bg-black/20 p-3">
      ...
      {#each [
        ...
        <label class="block rounded-md border border-white/10 bg-black/20 p-3">
          ...
          <select
            ...
            onchange={(event) => updateSwipeSetting(gesture.key, (event.currentTarget as HTMLSelectElement).value)}
          >
            ...
            {#each group.folders as folder (folder.id)}
              <label class="flex items-center justify-between gap-3 rounded-md border border-white/10 bg-white/[0.02] px-3 py-2">
                ...
                <select
                  ...
                  value={folder.role || ''}
                  onchange={(event) => saveFolderRole(folder.id, (event.currentTarget as HTMLSelectElement).value)}
                >
