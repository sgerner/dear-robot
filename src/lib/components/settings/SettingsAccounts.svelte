<script lang="ts">
  ...
  let {
    ...
    accountAction: (id: number, action: 'test' | 'enable' | 'disable' | 'delete') => void | Promise<void>;
  ...
  <section class="space-y-3 rounded-lg border border-white/10 bg-white/[0.03] p-3">
    ...
    <div class="relative">
      ...
      <div class="absolute right-2 top-2">
        <DictationButton targetId="google-oauth-scopes" activeTargetId={dictationTargetId} recording={dictationActive} unavailable={dictationUnavailable} level={dictationLevel} onToggle={toggleDictation} />
      </div>
    </div>
